...
try {
    ...
    $orgId = isset($currentUser['organization_id']) ? (int)$currentUser['organization_id'] : 0;
    $planFeatureChecker = getPlanFeatureChecker($pdo, $currentUser);

    // NEW: Initialize SubscriptionMiddleware
    require_once __DIR__ . '/SubscriptionMiddleware.php';
    $subscriptionMiddleware = getSubscriptionMiddleware($pdo, $planFeatureChecker, $orgId);
