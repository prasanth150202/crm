...
try {
    ...
    // 1. NEW: Check user limit before allowing invitation
    $userLimitCheck = $subscriptionMiddleware->canAddUser();
    if (!$userLimitCheck['allowed']) {
        ApiResponse::error($userLimitCheck['message'] . " Please upgrade to invite more users.", 403);
    }
    ...
    // 2. NEW: Validate features against the PLAN
    $planFeatures = $planFeatureChecker->getAllFeatures();
    $unsupportedFeatures = [];
    foreach ($features as $f) {
        if (!isset($planFeatures[$f])) {
            $unsupportedFeatures[] = $f;
        }
    }
    
    if (!empty($unsupportedFeatures)) {
        ApiResponse::error('The following features are not supported by your current plan: ' . implode(', ', $unsupportedFeatures) . '. Please upgrade to assign these features.', 403);
    }
    ...
    // Validate features exist (in global feature_knobs)
    ...
    $baseUrl = Env::get('APP_URL', 'http://localhost/crm-final');
