...
try {
    ...
    // NEW: Check for external webhooks in actions (Professional plan restriction)
    ...
    $subscriptionMiddleware = getSubscriptionMiddleware($pdo, $planFeatureChecker, $org_id);
    
    // Check if any action is a webhook action
    ...
    if ($hasWebhookAction) {
        $webhookCheck = $subscriptionMiddleware->canUseWebhooks();
        if (!$webhookCheck['allowed']) {
            http_response_code(403);
            echo json_encode([
                'success' => false,
                'error' => $webhookCheck['message'],
                'upgrade_required' => true,
                'required_plan' => $webhookCheck['required_plan']
            ]);
            exit;
        }
    }
