...
if ($name === '') {
    ApiResponse::error('Plan name is required', 400);
}
...
try {
    ...
    if ($hasTaxCol) {
        $stmt = $pdo->prepare(
            "INSERT INTO plans (name, description, base_price_monthly, base_price_yearly,
        ...
        $stmt->execute([
            ...
            $included_users, $price_per_additional_user_monthly, $price_per_additional_user_yearly,
            $trial_days, $currency, $is_custom, $custom_org_id, $tax_treatment
        ]);
    } else {
        $stmt = $pdo->prepare(
            "INSERT INTO plans (name, description, base_price_monthly, base_price_yearly,
        ...
        $stmt->execute([
            $name, $description, $base_price_monthly, $base_price_yearly,
            $included_users, $price_per_additional_user_monthly, $price_per_additional_user_yearly,
            $trial_days, $currency, $is_custom, $custom_org_id
        ]);
    }
    ...
    if ($feature_keys) {
        ...
        foreach ($feature_keys as $key) {
            $ins->execute([$planId, $key]);
        }
    }

    $pdo->commit();
    ApiResponse::success(['plan_id' => $planId, 'message' => 'Plan created successfully']);
