...
/* ------------------ APPLY MAPPING ------------------ */
$allowedStandard = ['name','first_name','last_name','email','phone','company','source','stage_id','lead_value','title','address','city','state','country','zip_code','website','description']; // Only columns that exist in leads table
$standard = [];
$custom   = [];

/**
 * Resolve dot-notation path in array (e.g., 'data.name')
 */
...
foreach ($mapping as $field => $sources) {
    ...
    foreach ($sources as $source) {
        if (preg_match('/^\{\{(.+)\}\}$/', $source, $m)) {
            $path = $m[1];
            $payloadValue = getValueByPath($data, $path);
...
file_put_contents($logFile . '.mapped', json_encode([
    'standard' => $standard,
    'custom' => $custom
], JSON_PRETTY_PRINT));

// Logic to combine first_name and last_name into name if name is missing
if (empty($standard['name']) && (!empty($standard['first_name']) || !empty($standard['last_name']))) {
    $fn = $standard['first_name'] ?? '';
    $ln = $standard['last_name'] ?? '';
    $standard['name'] = trim($fn . ' ' . $ln);
}

// Fallback for name if still empty (as leads table requires name NOT NULL)
if (empty($standard['name'])) {
    if (!empty($standard['email'])) {
        $standard['name'] = explode('@', $standard['email'])[0];
    } else {
        $standard['name'] = 'Lead from Webhook';
    }
}
...
try {
    ...
    if ($existingLeadId) {
        // Merge custom data if lead exists
        $stmt = $pdo->prepare('SELECT custom_data FROM leads WHERE id = ?');
        $stmt->execute([$existingLeadId]);
        $existingJson = $stmt->fetchColumn();
        $existingCustom = json_decode($existingJson ?: '{}', true) ?: [];
        $mergedCustom = array_merge($existingCustom, $custom);
        $params['custom_data'] = json_encode($mergedCustom);
