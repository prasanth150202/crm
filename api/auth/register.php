...
try {
        ...
        // Helper: assign only plan-specific features to user
        function assignAllPlanFeaturesToUser($pdo, $userId, $planId) {
            // Assign ONLY features from plan_features for this specific plan
            $stmt = $pdo->prepare("SELECT knob_key FROM plan_features WHERE plan_id = ?");
            $stmt->execute([$planId]);
            $features = $stmt->fetchAll(PDO::FETCH_COLUMN);
            if (!$features) {
                error_log('[register] No features found for plan_id=' . $planId);
                return;
            }
            foreach ($features as $knobKey) {
            ...
            error_log('[register] Assigned plan-specific features to user_permissions for user_id=' . $userId . ', plan_id=' . $planId . ', features=' . json_encode($features));
        }
    ...
    $stmt = $pdo->prepare("INSERT INTO organizations (name, current_plan_id) VALUES (?, ?)");
    $stmt->execute([$org_name, $plan_id]);
    ...
    if ($is_free_plan) {
        ...
        // Paid plan: create Razorpay subscription if credentials are available
        try {
            // Get Razorpay plan ID first
            ...
            if (empty($planId)) {
                error_log('[REGISTER] Razorpay plan ID not configured for plan: ' . $plan['name'] . ', billing: ' . $billing_interval);
                throw new Exception('Razorpay plan ID not configured');
            }
            ...
            // If Razorpay is not configured, allow registration to proceed with trial status
            // The subscription can be activated later when payment is configured
            error_log('[REGISTER] Razorpay not configured, proceeding with trial: ' . $razorpayError->getMessage());
            $razorpay_subscription_id = 'trial_' . $org_id . '_' . time();
        }
    }
    ...
    $stmt->execute([$org_id, $plan_id, $razorpay_subscription_id, $status, $billing_interval, $trial_starts_at, $trial_ends_at]);
