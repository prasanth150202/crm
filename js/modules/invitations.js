    ...
    async loadFeatures() {
        try {
            ...
            if (data && data.success) {
                // Get allowed features for this org/plan
                let allowed = [];
                if (App.planFeatures && Array.isArray(App.planFeatures)) {
                    allowed = App.planFeatures;
                } else if (typeof App.getPlanFeatures === 'function') {
                    allowed = await App.getPlanFeatures();
                } else if (window.planFeatures && Array.isArray(window.planFeatures)) {
                    allowed = window.planFeatures;
                }
                // Fallback: allow all if not found
                if (!allowed || allowed.length === 0) {
                    allowed = data.features.map(f => f.knob_key);
                }
                // Filter features to only those allowed by plan
                this.features = data.features.filter(f => allowed.includes(f.knob_key));
                this.renderFeatures();
            } else {
