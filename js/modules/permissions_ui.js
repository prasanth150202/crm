...
const PermissionsUI = {
    ...
    /**
     * Load current user's permissions from global state or backend
     */
    async loadUserPermissions() {
        // If window.userPermissions is already set by bootstrap, use it
        if (window.userPermissions && Object.keys(window.userPermissions).length > 0) {
            this.userPermissions = Object.keys(window.userPermissions).filter(key => window.userPermissions[key]);
            console.log('PermissionsUI using pre-loaded permissions:', this.userPermissions);
            return;
        }

        try {
            const response = await fetch(`${window.App.apiUrl}/auth/get_current_permissions.php`, {
                credentials: 'include'
            });
            ...
            if (data.success && data.permissions) {
                ...
                console.log('PermissionsUI loaded permissions dynamic:', this.userPermissions);
            }
        } catch (error) {
            console.error('Failed to load user permissions in UI module:', error);
        }
    },

    /**
     * Check if user has a specific permission (considering Plan)
     */
