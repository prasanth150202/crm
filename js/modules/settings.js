...
Object.assign(App, {
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-200">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Set the currency used for lead values across your organization</p>
                    </div>
                    <div class="p-6 flex items-center gap-4">
                        <select id="orgCurrencySelect" class="block border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm border p-2 bg-white">
                        ...
                        <button onclick="App.saveCurrency()" class="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700">
                            Save Currency
                        </button>
                    </div>
                </div>
                ...
                <!-- NEW: Plan & Subscription Details Section -->
                <div class="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden" id="planDetailsSection">
                    <div class="px-6 py-4 border-b border-gray-200">
                        <h3 class="text-lg font-medium text-gray-900">Plan & Subscription</h3>
                        <p class="text-sm text-gray-500 mt-1">Details about your current organization plan and features</p>
                    </div>
                    <div class="p-6" id="planDetailsContent">
            ...
            modal.querySelector('#editFieldOptionsForm').addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if (result && result.success) {
                        ...
                        modal.remove();
                        // Clear kanban stage cache so pipeline reflects new stage order immediately
                        this._cachedStageOptions = null;
    ...
    async loadCurrencySetting() {
        const select = document.getElementById('orgCurrencySelect');
        if (!select) return;
        try {
            const result = await this.api('/settings/currency.php');
            if (result && result.success) {
                select.value = result.currency || 'USD';
            }
        } catch (e) {
    ...
    async loadPlanDetails() {
        const container = document.getElementById('planDetailsContent');
        if (!container) return;

        try {
            const result = await this.api('/subscriptions/get_plan_details.php');
            if (result && result.success) {
                this.renderPlanDetails(result.data);
            } else {
    ...
    renderPlanDetails(data) {
        ...
        container.innerHTML = `
            <div class="space-y-6">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-xl font-bold text-gray-900">${plan.name} Plan</h4>
                        <p class="text-sm text-gray-500">Current subscription for this organization</p>
                    </div>
                    <div class="text-right">
                        <span class="px-3 py-1 text-sm font-bold rounded-full bg-green-100 text-green-800 uppercase">
                            ${plan.base_price_monthly == 0 ? 'Free' : 'Active'}
                        </span>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 rounded-lg p-4 border border-gray-100">
                    <div class="flex justify-between items-center mb-2">
                        <h5 class="text-sm font-semibold text-gray-700">User Usage</h5>
                        <span class="text-sm font-medium text-gray-900">${user_count} / ${user_limit || '∞'} Users</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div class="${progressBarColor} h-2 rounded-full transition-all duration-500" style="width: ${Math.min(progress, 100)}%"></div>
                    </div>
                    <p class="text-xs text-gray-500 mt-2">
                        ${user_limit ? `You are using ${user_count} out of ${user_limit} included user seats.` : 'Unlimited user seats included in your plan.'}
                    </p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mt-6">
                    <div class="p-4 rounded-lg bg-blue-50 border border-blue-100">
                        <p class="text-xs font-bold text-blue-600 uppercase tracking-wider mb-1">Pricing Model</p>
                        <p class="text-sm font-medium text-blue-900">${plan.base_price_monthly == 0 ? 'Forever Free' : 'Usage Based'}</p>
                    </div>
                    <div class="p-4 rounded-lg bg-purple-50 border border-purple-100">
                        <p class="text-xs font-bold text-purple-600 uppercase tracking-wider mb-1">Billing Support</p>
                        <p class="text-sm font-medium text-purple-900">Razorpay Protected</p>
                    </div>
                </div>
