...
export const Meetings = {
    ...
    async saveMeeting(event) {
        ...
        try {
            ...
            if (result.success) {
                ...
                // Context-aware refresh after save:
                const leadMeetingsSection = document.getElementById('leadMeetingsSection');
                const onMeetingsPage = document.getElementById('pageTitle') && document.getElementById('pageTitle').textContent === 'Meetings';

                if (leadMeetingsSection && typeof App.renderLeadMeetings === 'function') {
                    // Lead detail panel is open — refresh just its meetings section
                    const leadNameEl = document.getElementById('meetingLeadName');
                    const leadName = leadNameEl ? leadNameEl.value : '';
                    await App.renderLeadMeetings(leadId, leadName);
                } else if (onMeetingsPage) {
                    // Full meetings page — reload the list
                    await this.loadMeetings();
                }
                // else: called from leads table — toast is enough, no redirect
            } else {
     ...
     * Delete meeting
     */
    async deleteMeeting(meetingId, skipReload = false) {
