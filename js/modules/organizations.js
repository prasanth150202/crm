...
Object.assign(App, {
    async loadOrganizations() {
        document.getElementById("appContent").innerHTML =
            '<div class="text-center py-10"><p class="text-gray-500">Loading Organizations...</p></div>';
        try {
            ...
            if (orgs && orgs.success) {
                ...
                const errorMsg =
                    orgs && orgs.error ? orgs.error : "Unknown error";
                document.getElementById("appContent").innerHTML = `
    ...
    async toggleOrganizationStatus(orgId, activate) {
        ...
        if (!confirm(`Are you sure you want to ${action} this organization?`))
            return;
        try {
            const result = await this.api("/organizations/update.php", "POST", {
                org_id: orgId,
                is_active: activate,
            });
    ...
    async deleteOrganization(orgId, orgName) {
        ...
        try {
            const result = await this.api("/organizations/delete.php", "POST", {
                org_id: orgId,
            });
            if (result && result.success) {
    ...
    async openCreateOrgModal() {
        ...
        let plans = [];
        try {
            const res = await fetch("api/plans/list.php"); // relative to /crm-final/
        ...
        document.getElementById("cancelCreateOrgBtn").onclick = () =>
            modal.remove();
                    ...
                    if (result.razorpay_subscription_id) {
                        ...
                        const razorpayKey =
                            (window.AppData &&
                                ...
                                window.AppData.config.razorpayKeyId) ||
                            "";
                        if (!razorpayKey) {
                            alert(
                                "Razorpay key not configured in environment variables.",
                            );
                            this.loadOrganizations();
                            return;
                        }
                        const options = {
                            ...
                            prefill: {
                                name:
                                    (window.AppData &&
                                        window.AppData.user &&
                                        window.AppData.user.name) ||
                                    "",
                                email:
                                    (window.AppData &&
                                        window.AppData.user &&
                                        window.AppData.user.email) ||
                                    "",
                            },
                            theme: { color: "#2563eb" },
                        };
    ...
    async switchOrganization(orgId, orgName) {
        try {
            const result = await this.api("/organizations/switch.php", "POST", {
                org_id: orgId,
            });
            if (result.success) {
    ...
    async handleOrgChange(value) {
        try {
            const stmt = await this.api("/organizations/switch.php", "POST", {
                org_id: parseInt(value),
            });
